...
export function AlumniCard({ alumni, variant = "grid", onSaved, index = 0, onSelect }: {
  ...
  if (variant === "swipe") {
    return (
      <div className="relative flex h-full w-full flex-col overflow-hidden rounded-xl bg-white border border-[var(--color-border)]">
        ...
        <div className="flex flex-1 flex-col justify-between p-5">
          <div>
            <h3 className="text-lg font-bold text-[var(--color-text)]">{alumni.fullName}</h3>
            <p className="mt-1 flex items-center gap-1.5 text-sm text-[var(--color-text-secondary)]">
              <GraduationCap size={14} className="shrink-0" />
              {alumni.universityName}
            </p>
            <p className="mt-1 text-xs text-[var(--color-text-tertiary)]">{alumni.country} · {alumni.course}</p>
            <p className="mt-2 line-clamp-2 text-sm leading-5 text-[var(--color-text-secondary)]">{alumni.bio ?? "Ready to share practical advice."}</p>
          </div>
          <div className="flex items-center gap-2 pt-3 border-t border-[var(--color-border-light)] mt-3">
            <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-surface)] px-2 py-0.5 text-[11px] font-medium text-[var(--color-text-secondary)]">
              <Clock3 size={11} />
              {responseTime != null ? `${Math.round(responseTime)}h` : "—"}
            </span>
            {alumni.ratingAvg != null && (
              <span className="ml-auto flex items-center gap-1 text-xs font-semibold text-[var(--color-text)]">
  ...
  return (
    <div onClick={() => onSelect?.(alumni.id)} className="group cursor-pointer">
      <div
        className="bg-white rounded-[var(--radius-lg)] border border-[var(--color-border)] overflow-hidden card-elevation"
      ...
      >
        ...
        <div className="relative" style={{ aspectRatio: "4 / 3" }}>
          ...
          <button
            ...
            onClick={toggle}
            disabled={saving}
            className="absolute top-3 right-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-white/85 backdrop-blur-sm shadow-sm transition-all hover:scale-110 hover:shadow-md disabled:opacity-50"
          >
          ...
          {alumni.ratingAvg != null && alumni.ratingAvg >= 4.5 && (
            <div className="absolute top-3 left-3 flex items-center gap-1 rounded-full bg-white/85 backdrop-blur-sm px-2 py-0.5 shadow-sm">
              ...
              <span className="text-[11px] font-bold text-[var(--color-text)]">{alumni.ratingAvg.toFixed(1)}</span>
            </div>
          )}

          {/* Verified badge — bottom-left */}
          <div className="absolute bottom-3 left-3">
            <span className="inline-flex items-center gap-1 rounded-full bg-white/85 backdrop-blur-sm px-2 py-0.5 text-[11px] font-medium text-[var(--color-text)] shadow-sm">
        ...
        <div className="p-5">
          ...
          <div className="flex items-start justify-between gap-3 mb-1">
            <h3 className="text-[16px] font-semibold text-[var(--color-text)] leading-snug group-hover:text-[var(--color-primary)] transition-colors line-clamp-1">{alumni.fullName}</h3>
            {alumni.ratingAvg != null && (
              <span className="flex shrink-0 items-center gap-1 text-sm font-semibold text-[var(--color-text)]">
          ...
          {/* University */}
          <p className="flex items-center gap-1.5 text-[13px] text-[var(--color-text-secondary)] mb-1">
            <GraduationCap size={13} className="shrink-0 text-[var(--color-text-tertiary)]" />
            <span className="truncate">{alumni.universityName}</span>
          </p>

          {/* Location + year */}
          <p className="flex items-center gap-1 text-xs text-[var(--color-text-tertiary)] mb-3">
          ...
          <div className="flex items-center gap-2 mb-3">
            <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-surface)] px-2.5 py-1 text-[11px] font-medium text-[var(--color-text-secondary)]">
              <Clock3 size={11} />
              {responseTime != null
          ...
          {/* Bio — 2 line max */}
          <p className="line-clamp-2 text-[13px] leading-5 text-[var(--color-text-secondary)] mb-4">
          ...
          <div className="flex items-center justify-between pt-3 border-t border-[var(--color-border-light)]">
            <div className="flex items-center gap-2">
              {hasSessions && (
                <span className="inline-flex items-center gap-1 rounded-md bg-[var(--color-surface)] px-2.5 py-1 text-[11px] font-medium text-[var(--color-text-secondary)]">
                  <Video size={11} />
            ...
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-[var(--color-primary)] transition-all group-hover:bg-[var(--color-primary-light)]">
