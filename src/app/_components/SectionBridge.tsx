...
// Each card sits at a slightly different scroll-linked depth for a real parallax feel.
const PARALLAX_DEPTH = [70, 40, 90];
    ...
    >
      ...
      <div className="relative mx-auto max-w-[1500px] px-6 py-16 sm:px-10 sm:py-24 lg:px-16 lg:py-32">
        <div className="max-w-3xl">
          <ScrollReveal as="p" y={16} className="text-[11px] font-semibold uppercase tracking-[0.34em] text-white/30">
            From finding to talking
          </ScrollReveal>
          <ScrollReveal as="h2" delay={0.08} className="mt-5 text-[clamp(2rem,5.5vw,4.5rem)] leading-[0.92] tracking-[-0.04em] text-white font-bold font-heading">
            The search should not end at a page.
            <span className="text-coral"> It should open a conversation.</span>
          </ScrollReveal>
          <ScrollReveal as="p" delay={0.14} className="mt-5 max-w-2xl text-base leading-7 text-white/40 sm:text-lg">
        ...
        <div className="mt-14 grid gap-5 lg:mt-20 lg:grid-cols-3">
          {steps.map(({ icon: Icon, label, title, body, gradient }, index) => (
            <ScrollReveal
              ...
              as="div"
              parallax
              parallaxAmount={PARALLAX_DEPTH[index]}
            >
              <article className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02] p-7 transition-all duration-300 hover:-translate-y-1.5 hover:border-white/[0.12] hover:shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
                {/* Gradient accent bar */}
